...
def boundary_attack(model, oracle, target_, question_, answer_, k):
    ...
    plt.imshow(adversarial_img, cmap="gray")
    plt.show()
    Image.fromarray((255 * adversarial_img).astype(np.uint8)).save('output/image.png')
    ...
    for i in tqdm(range(k)):
        ...
        if temp_adversarial_imgs is None:  # point of convergence
            continue
        # downwards step
        temp_adversarial_imgs = np.array([adaptive_epsilon_descent_(x) for x in temp_adversarial_imgs], dtype=np.float64)
        # print('\n')
...
if __name__ == "__main__":
    ...
    final = boundary_attack(model, eval_model, target, question, target_answer, 2)
